...
def YOLO(in123, in1234):
    ...
    configPath = "./cfg/yolov4-obj.cfg"                                 # Path to cfg
    weightPath = "./yolov4-obj_best.weights"                                 # Path to weights
    metaPath = "./cfg/obj.data"                                    # Path to meta data
        ...
        if len(detections)>0:
            for detection in detections:
                ...
                try:
                    ...
                    print(classIndex)
                    if probabilityValue > threshold1:
                        name123456 = str(getCalssName(classIndex))
                        print(f'{name123456}')
                    else:
                        o = 2
                    ...
                    # imgresized1 = cv2.resize(imgcroppped1,(w1, h1), interpolation=cv2.INTER_LANCZOS4)
                    # kernel = np.array([[0, -1, 0],
                    #                    [-1, 5, -1],
                    #                    [0, -1, 0]])
                    # #image_sharp = cv2.filter2D(imgresized1, -1, kernel)
                    # winname = "Cropped"
                    # cv2.namedWindow(winname)
                    # cv2.moveWindow(winname, 40, 30)
                    #
                    # cv2.imshow(winname, imgresized1)
                    # cv2.waitKey(0)
                    #
                    #
                    #
                    # imggray = cv2.cvtColor(imgresized1, cv2.COLOR_BGR2GRAY)
                    #
                    # #imgAdaptiveThre = cv2.adaptiveThreshold(imggray, 255, 1, 1, 7, 2)
                    # #imgAdaptiveThre = cv2.bitwise_not(imggray)
                    # imgAdaptiveThre = cv2.medianBlur(imggray, 3)
                    # cv2.imshow(winname, imgAdaptiveThre)
                    # cv2.waitKey(0)
                    # # imgblur = cv2.GaussianBlur(imgchange, (5,5), 1)
                    # # cv2.imshow(winname, imgblur)
                    # # cv2.waitKey(0)
                    # #
                    # # cv2.imshow(winname, imgchange)
                    # # cv2.waitKey(0)
                    # print(pytesseract.image_to_string(imgAdaptiveThre))
                    # print('tesseract')
